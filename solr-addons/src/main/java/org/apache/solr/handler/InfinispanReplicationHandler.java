...
public class InfinispanReplicationHandler extends ReplicationHandler {
	...
	private static final Logger LOG = LoggerFactory.getLogger(InfinispanReplicationHandler.class.getName());
	...
	public void handleRequestBody(SolrQueryRequest req, SolrQueryResponse rsp)
		...
		if (command.equals(CMD_INDEX_VERSION)) {
			IndexCommit commitPoint = indexCommitPoint; // make a copy so it
														// won't change
			if (commitPoint != null && replicationEnabled.get()) {
				//
				// There is a race condition here. The commit point may be
				// changed / deleted by the time
				// we get around to reserving it. This is a very small window
				// though, and should not result
				// in a catastrophic failure, but will result in the client
				// getting an empty file list for
				...
				// This happens when replication is not configured to happen
				// after startup and no commit/optimize
				// has happened yet.
			...
			if (!isSlave && masterUrl == null) {
				...
				rsp.add("message",
						"No slave configured or no 'masterUrl' Specified");
				return;
			}
	...
	private void getFileChecksum(SolrParams solrParams, SolrQueryResponse rsp) {
		...
		rsp.add(CHECKSUM,
				getCheckSums(solrParams.getParams(FILE), dir, checksum));
		dir = new File(core.getResourceLoader().getConfigDir());
		rsp.add(CONF_CHECKSUM,
				getCheckSums(solrParams.getParams(CONF_FILE_SHORT), dir,
						checksum));
	}

	private Map<String, Long> getCheckSums(String[] files, File dir,
			Checksum checksum) {
	...
	void doFetch(SolrParams solrParams) {
		String masterUrl = solrParams == null ? null : solrParams
				.get(MASTER_URL);
	...
	private void getFileList(SolrParams solrParams, SolrQueryResponse rsp) {
		...
		try {
			...
			// use a set to workaround possible Lucene bug which returns same
		...
		rsp.add(CONF_FILES,	getConfFileInfoFromCache2(confFileNameAlias, confFileInfoCache));
	}
	 ...
	 */
	private List<Map<String, Object>> getConfFileInfoFromCache2(
	...
	private long[] getIndexVersion() {
		...
		try {
			version[0] = searcher.get().getReader().getIndexCommit()
					.getVersion();
			version[1] = searcher.get().getReader().getIndexCommit()
					.getGeneration();
		} catch (IOException e) {
	...
	public NamedList getStatistics() {
		...
		if (core != null) {
			...
			if (isMaster) {
				...
				list.add("replicationEnabled",
						String.valueOf(replicationEnabled.get()));
			}
		}
		return list;
	}
		...
		if (showSlaveDetails && snapPuller != null) {
			...
			if (snapPuller.getNextScheduledExecTime() != null
				...
				slave.add(NEXT_EXECUTION_AT,
						new Date(snapPuller.getNextScheduledExecTime())
								.toString());
			...
			if (isReplicating) {
				try {
					...
					for (Map<String, Object> file : snapPuller
							.getFilesToDownload()) {
					...
					Map<String, Object> currentFile = snapPuller
							.getCurrentFile();
					...
					if (currentFile != null) {
						...
						if (currentFile.containsKey("bytesDownloaded")) {
							...
							if (currFileSize > 0)
								percentDownloaded = (currFileSizeDownloaded * 100)
										/ currFileSize;
						}
					}
					...
					if (snapPuller.getReplicationStartTime() > 0) {
						slave.add("replicationStartTime",
								new Date(snapPuller.getReplicationStartTime())
										.toString());
					}
					long elapsed = getTimeElapsed(snapPuller);
					...
					if (bytesDownloaded > 0)
						estimatedTimeRemaining = ((bytesToDownload - bytesDownloaded) * elapsed)
								/ bytesDownloaded;
					float totalPercent = 0;
					long downloadSpeed = 0;
					if (bytesToDownload > 0)
						totalPercent = (bytesDownloaded * 100)
								/ bytesToDownload;
					if (elapsed > 0)
					...
					slave.add("currentFileSizeDownloaded",
							readableSize(currFileSizeDownloaded));
					slave.add("currentFileSizePercent",
							String.valueOf(percentDownloaded));
					slave.add("bytesDownloaded", readableSize(bytesDownloaded));
					slave.add("totalPercent", String.valueOf(totalPercent));
					slave.add("timeRemaining",
							String.valueOf(estimatedTimeRemaining) + "s");
					slave.add("downloadSpeed", readableSize(downloadSpeed));
				} catch (Exception e) {
					LOG.error("Exception while writing replication details: ",
							e);
				}
			}
		}
	...
	private long getTimeElapsed(SnapPuller snapPuller) {
		long timeElapsed = 0;
		if (snapPuller.getReplicationStartTime() > 0)
			timeElapsed = (System.currentTimeMillis() - snapPuller
					.getReplicationStartTime()) / 1000;
		return timeElapsed;
	}
	...
	Properties loadReplicationProperties() {
		...
		try {
			File f = new File(core.getDataDir(),
					SnapPuller.REPLICATION_PROPERTIES);
			if (f.exists()) {
	...
	void refreshCommitpoint() {
		...
		if (replicateOnCommit
				|| (replicateOnOptimize && commitPoint.isOptimized())) {
	...
	public void inform(SolrCore core) {
		...
		if (enableMaster) {
			...
			if (includeConfFiles != null
					&& includeConfFiles.trim().length() > 0) {
				...
				for (String file : files) {
					...
					confFileNameAlias.add(strs[0], strs.length > 1 ? strs[1]
							: null);
				}
			...
			boolean backupOnOptimize = !backupOnCommit
					&& backup.contains("optimize");
			List replicateAfter = master.getAll(REPLICATE_AFTER);
			replicateOnCommit = replicateAfter.contains("commit");
			replicateOnOptimize = !replicateOnCommit
					&& replicateAfter.contains("optimize");

			// if we only want to replicate on optimize, we need the deletion
			// policy to
			...
			if (replicateOnOptimize || backupOnOptimize) {
				core.getUpdateHandler()
						.registerOptimizeCallback(
								getEventListener(backupOnOptimize,
										replicateOnOptimize));
			}
			if (replicateOnCommit || backupOnCommit) {
				replicateOnCommit = true;
				core.getUpdateHandler().registerCommitCallback(
						getEventListener(backupOnCommit, replicateOnCommit));
			}
			if (replicateAfter.contains("startup")) {
				...
				try {
					...
					if (reader != null && reader.getIndexCommit() != null
						...
						try {
							if (replicateOnOptimize) {
								Collection<IndexCommit> commits = IndexReader
	...
	private void registerFileStreamResponseWriter() {
		core.registerResponseWriter(FILE_STREAM,
				new BinaryQueryResponseWriter() {
					public void write(OutputStream out,
							SolrQueryRequest request, SolrQueryResponse resp)
							throws IOException {
						FileStream stream = (FileStream) resp.getValues().get(
								FILE_STREAM);
						stream.write(out);
					}
		...
		return new SolrEventListener() {
			...
			public void postCommit() {
				...
				if (getCommit) {
					 ...
					 * if (indexCommitPoint != null) {
					 * core.getDeletionPolicy().saveCommitPoint
					 * (indexCommitPoint.getVersion()); } if(oldCommitPoint !=
					 * null){ core.getDeletionPolicy().releaseCommitPoint(
					 * oldCommitPoint.getVersion()); }
					 ***/
				}
				if (snapshoot) {
					try {
